  ...
  useEffect(() => {
    const obs = new IntersectionObserver(
      (entries) => {
        entries.forEach((e) => {
          if (e.isIntersecting) {
            e.target.querySelectorAll('.pr-card, .anim-up').forEach((el) => el.classList.add('visible'));
          }
        });
      },
      { threshold: 0.08 }
    ...
    <section ref={sectionRef} className="pr-sec section-pad" id="pricing" aria-labelledby="pricing-title">
      ...
      <div className="container pr-wrap">
        ...
        <div className="pr-header anim-up">
          ...
          <h2 className="pr-title" id="pricing-title">
            Integrate the Power of <span className="pr-grad">People</span> with the Intelligence of <span className="pr-grad">Technology</span>
          </h2>
        </div>
        ...
        <div className="pr-tabs">
          {tabs.map((t) => (
            <button
              key={t.key}
              className={`pr-tab ${period === t.key ? 'pr-tab-on' : ''}`}
        ...
        <div className="pr-grid">
          {plans.map((plan, i) => (
            <div
              key={plan.tier}
              className={`pr-card ${plan.popular ? 'pr-popular' : ''}`}
              style={{
                animationDelay: `${i * 0.12}s`,
                ['--pc' as string]: plan.color,
                ['--pcr' as string]: plan.colorRgb,
              }}
            >
              {/* Popular badge */}
              ...
              <div className="pr-amounts">
                <div className="pr-row">
                  <span>Price</span>
                  <span className="pr-val">{plan.prices[period]}</span>
                </div>
              </div>
      ...
      <style jsx>{`
        ...
        .pr-orb {
          ...
          filter: blur(100px);
          pointer-events: none;
          animation: prDrift 12s ease-in-out infinite;
          z-index: 0;
        }
        ...
        .pr-grad {
          ...
          animation: prShim 3s linear infinite;
        }
        ...
        .pr-tab-on {
          background: var(--gradient-btn, linear-gradient(135deg, #6DB33F, #4A8C1F));
          color: #fff;
        ...
        .pr-card.visible {
          animation: prCardUp .7s ease forwards;
        }

        .pr-card:hover {
          transform: translateY(-10px) scale(1.02);
          border-color: rgba(var(--pcr), 0.4);
          box-shadow:
            0 0 50px rgba(var(--pcr), 0.1),
        ...
        .pr-card:hover .pr-icon {
          ...
          box-shadow: 0 0 28px rgba(var(--pcr), 0.25);
          background: rgba(var(--pcr), 0.18);
        }
        ...
        .pr-card:hover .pr-tier {
          text-shadow: 0 0 14px rgba(var(--pcr), 0.4);
        }
        ...
        .pr-card:hover .pr-desc { color: #b8bfb8; }
        ...
        .pr-card:hover .pr-val {
          color: var(--pc);
          text-shadow: 0 0 12px rgba(var(--pcr), 0.3);
        ...
        .pr-card:hover .pr-feat { color: #b0b7b0; }
        ...
        .pr-card:hover .pr-feat i {
          transform: scale(1.2);
          filter: drop-shadow(0 0 4px rgba(143,212,94,0.4));
        }
        ...
        @media (max-width: 1024px) {
          ...
          .pr-grid .pr-card:last-child { grid-column: 1 / -1; max-width: 400px; margin: 0 auto; width: 100%; }
        }
        @media (max-width: 640px) {
          .pr-grid { grid-template-columns: 1fr; }
          .pr-grid .pr-card:last-child { max-width: 100%; }
          .pr-tabs { flex-wrap: wrap; }
          .pr-tab { padding: 8px 20px; font-size: .8rem; }
        }
      `}</style>
    </section>
  );
